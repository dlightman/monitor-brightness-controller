...
public sealed class MonitorInterop : IMonitorInterop
{
    ...
    /// <inheritdoc />
    public Result<int> GetGamma(IntPtr physicalMonitorHandle)
    {
        if (!NativeMethods.GetVCPFeatureAndVCPFeatureReply(
                physicalMonitorHandle,
                NativeMethods.VcpGamma,
                out _,
                out uint current,
                out uint maximum))
        {
            return Result<int>.Failure("Failed to read gamma from the monitor via DDC/CI.");
        }
    ...
    /// <inheritdoc />
    public Result<Unit> SetGamma(IntPtr physicalMonitorHandle, int value)
    {
        if (value < 0 || value > 100)
        {
            return Result<Unit>.Failure($"Gamma value '{value}' is out of the valid range [0, 100].");
        }
        ...
        if (!NativeMethods.SetVCPFeature(physicalMonitorHandle, NativeMethods.VcpGamma, rawValue))
        {
            return Result<Unit>.Failure("Failed to set gamma on the monitor via DDC/CI.");
        }
