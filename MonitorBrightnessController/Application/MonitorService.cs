...
public sealed class MonitorService : IMonitorService
{
    ...
    public IReadOnlyList<MonitorState> DetectMonitors()
    {
        ...
        // Hardware step: read current brightness and gamma for controllable monitors.
        for (int i = 0; i < states.Count; i++)
        {
            ...
            {
                // Unsupported monitors have unknown brightness/gamma and no controls (Req 1.3/1.4).
                states[i] = state with { CurrentBrightness = null, CurrentGamma = null };
                continue;
            }

            Result<int> brightnessRead = _interop.GetBrightness(state.PhysicalHandle);
            state = brightnessRead.IsSuccess
                ? state with { CurrentBrightness = brightnessRead.Value, ErrorMessage = null }
                : state with { CurrentBrightness = null, ErrorMessage = brightnessRead.Error };

            Result<int> gammaRead = _interop.GetGamma(state.PhysicalHandle);
            state = gammaRead.IsSuccess
                ? state with { CurrentGamma = gammaRead.Value, ErrorMessage = null }
                : state with { CurrentGamma = null, ErrorMessage = gammaRead.Error };
    ...
    {
        if (gammaValue < MinBrightness || gammaValue > MaxBrightness)
        {
            return Result<Unit>.Failure(
                $"Invalid gamma value '{gammaValue}': must be integer {MinBrightness}-{MaxBrightness}");
        }
        ...
        MonitorState target = _monitors[slot];
        Result<Unit> result = _interop.SetGamma(target.PhysicalHandle, gammaValue);

        _monitors[slot] = result.IsSuccess
            ? target with { CurrentGamma = gammaValue, ErrorMessage = null }
            : target with { ErrorMessage = result.Error };
    ...
    public Result<int> GetGamma(int monitorIndex)
    {
        ...
        MonitorState target = _monitors[slot];
        Result<int> result = _interop.GetGamma(target.PhysicalHandle);

        _monitors[slot] = result.IsSuccess
            ? target with { CurrentGamma = result.Value, ErrorMessage = null }
            : target with { CurrentGamma = null, ErrorMessage = result.Error };
