...
public sealed class ProfileManager : IProfileManager
{
        ...
        IReadOnlyDictionary<string, int>? gammaMap)
    {
        ...
        if (gammaMap is not null && !TryValidateGammaMap(gammaMap, out string? gammaError))
        {
            return Result<Unit>.Failure(gammaError!);
        }
        ...
        profiles.Add(new Profile
        {
            ...
            MonitorGammaMap = gammaMap is not null ? new Dictionary<string, int>(gammaMap) : null,
        });
    ...
    public Result<Unit> ApplyProfile(string name, IMonitorService monitorService)
    {
        ...
        // Collect the set of device paths that appear in at least one mapping and are connected.
        var connectedMappedPaths = new HashSet<string>(StringComparer.Ordinal);
        var errors = new List<string>();

        // Apply brightness for each connected monitor in the brightness map.
        foreach (KeyValuePair<string, int> entry in profile.MonitorBrightnessMap)
        {
            ...
            connectedMappedPaths.Add(entry.Key);
            Result<Unit> result = monitorService.SetBrightness(target.MonitorIndex, entry.Value);
            if (!result.IsSuccess)
            {
                errors.Add(result.Error ?? $"Failed to set brightness on monitor {target.MonitorIndex}");
            }
        }

        // Apply gamma for each connected monitor in the gamma map (only if gamma map is not null).
        // A null gamma map means legacy profile — skip gamma entirely (Req 8.2).
        if (profile.MonitorGammaMap is not null)
        {
            foreach (KeyValuePair<string, int> entry in profile.MonitorGammaMap)
            {
                if (!byDevicePath.TryGetValue(entry.Key, out MonitorState? target))
                {
                    continue;
                ...
                connectedMappedPaths.Add(entry.Key);
                Result<Unit> result = monitorService.SetGamma(target.MonitorIndex, entry.Value);
                if (!result.IsSuccess)
                {
                    errors.Add(result.Error ?? $"Failed to set gamma on monitor {target.MonitorIndex}");
                }
            }
        }

        // If none of the profile's mapped monitors are connected, the apply fails (Req 6.6).
        if (connectedMappedPaths.Count == 0)
        {
            return Result<Unit>.Failure(
                $"Profile '{profile.Name}' has no mapped monitors that are currently connected.");
        }

        // At least one connected monitor was targeted: record the applied profile.
    ...
    private static bool TryValidateGammaMap(IReadOnlyDictionary<string, int> map, out string? error)
    {
        foreach (KeyValuePair<string, int> entry in map)
        {
            if (entry.Value < 0 || entry.Value > 100)
            {
                error = $"Invalid gamma value '{entry.Value}' for monitor '{entry.Key}': must be integer 0-100";
                return false;
            }
        }
